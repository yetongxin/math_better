...
for fold in range(5):
    ...
    for epoch in range(n_epochs):
        ...
        loss_total = 0
        input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches = prepare_ex_train_batch(train_pairs, batch_size, output_lang, source_rate)
        print("fold:", fold + 1)
        print("epoch:", epoch + 1)
        start = time.time()
        ...
        if (epoch+1) % 10 == 0 or epoch > n_epochs - 5:
            ...
            torch.save(merge.state_dict(), "models/merge_da")
            if epoch == n_epochs - 1:
