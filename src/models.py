...
class EncoderSeq3(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        super(EncoderSeq3, self).__init__()
...
class EncoderSeq4(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        self.speech_liner = nn.Linear(len(flag_dict), embedding_size)
        self.speech_dropout = nn.Dropout(0.5)
        self.num_pos_linear = nn.Linear(1, embedding_size)
        self.num_pos_dropout = nn.Dropout(0.5)

    def forward(self, input_seqs, input_lengths, flag, parsed_num_pos, evaluating=False,hidden=None):
        # if evaluating:
        #   print('evaluating mode:', input_seqs, input_seqs.shape, input_lengths, flag.shape)
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        ...
        # flag2 = self.speech_liner(flag)
        # flag2 = self.speech_dropout(flag2)
        # print('parsed num pos shape', parsed_num_pos.shape, parsed_num_pos.unsqueeze(2).shape)
        num_pos = self.num_pos_linear(parsed_num_pos.unsqueeze(2))
        num_pos = self.num_pos_dropout(num_pos)
        embedded = embedded + 0.5 * num_pos
        # rnn1
        packed = torch.nn.utils.rnn.pack_padded_sequence(embedded, input_lengths)
        pade_hidden = hidden
        pade_outputs, pade_hidden = self.rnn1(packed, pade_hidden)
